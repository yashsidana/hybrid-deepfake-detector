...
class TemporalClassifier(nn.Module):
    ...
    A per-frame ResNet-18 backbone (frozen, ImageNet-pretrained) extracts
    spatial features from each frame in the sequence independently; an LSTM
    then models how those features evolve across the 16-frame sequence. The
    LSTM's final hidden state is the temporal embedding.
    ...
    def forward(self, x):
        ...
        with torch.no_grad():
            # Backbone is frozen — no need to track gradients through it.
